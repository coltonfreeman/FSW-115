//GET Data Section
function getData() {
    axios.get('http://api.bryanuniversity.edu/coltonfreeman/list')
...
function createToDo(data) {
    ...
    for (let i = data.length - 1; i >= 0; i--) {
        ...
        checkbox.addEventListener('click', e => {
            ...
            if (e.target.checked) {
                ...
                axios.put(`http://api.bryanuniversity.edu/coltonfreeman/list/${parentNode.id}`, { isComplete: true })
                    .then(res => console.log(res))
                    .catch(err => console.log(err))
            } else {
                e.target.parentNode.style.textDecoration = 'none';
                axios.put(`http://api.bryanuniversity.edu/coltonfreeman/list/${parentNode.id}`, { isComplete: false })
                    .then(res => console.log(res))
                    .catch(err => console.log(err))
            }
        });
        ...
        updateButton.addEventListener('click', (e) => {
            ...
            axios.put(`http://api.bryanuniversity.edu/coltonfreeman/list/${itemId.id}`)
                .then(_res => getData())
                .catch(err => console.log(err))
        });
    };
    ...
    newData.addEventListener('submit', e => {
        ...
        axios.put(`http://api.bryanuniversity.edu/coltonfreeman/list/${itemId}`, updateTodo)
            .then(_res => getData())
            .catch(err => console.log(err))
    });
};